...
public class Operation {
	...
	public static int[] calculate(int[] cards, long dead, int cardsNumber)
	{
		int counts[] = null;
		//Hand h = new Hand ();
		switch (cardsNumber)
		...
		break;
	    case 3:
		counts = Operation.runThreeCard (cards, dead);
		break;
	    case 4:
		counts = Operation.runFourCard (cards, dead);
		break;
	    case 5:
	...
	for (int i=0; i<52; i++)
	{
		...
		if (lastProgress!=progress){
			...
			OddsCalculatorScreen.setProgressGauge(progress);
		}
	    if (((one<<i)&dead) == 0)
		for (int j=i+1; j<52; j++)
		    if (((one<<j)&dead) == 0)
